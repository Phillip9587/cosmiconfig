...
import { TempDir, isNotMjs } from './util.js';
...
describe('finds .foorc.json in first searched dir', () => {
  beforeEach(() => {
    temp.createFile('a/b/c/d/e/f/.foorc.json', JSON.stringify({ found: true }));
  });
...
describe('custom loaders allow non-default file types', () => {
  const loadThings = (_: any, content: any) => {
    return {
      things: content
        .split('\n')
        .map((x: any) => x.trim())
